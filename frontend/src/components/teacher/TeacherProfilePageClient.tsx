...
import { getTeacherCourses, getAllTeacherStudents } from "@/lib/auth-actions";

interface Props {
  initialUser: UserType;
}

export function TeacherProfilePageClient({ initialUser }: Props) {
  ...
  const fetchTeacherStats = async () => {
    try {
      ...
      const coursesWithRating = courses.filter(
        (course) => course.rating !== undefined && course.rating > 0
      );
      const averageRating =
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage your teaching profile and account information
          </p>
        </div>
        {!isEditing ? (
          <Button
              ...
              <div className="relative mb-4">
                <div
                  className={`w-32 h-32 mx-auto rounded-full overflow-hidden border-4 border-accent/20 ${
                    !isEditing && (imagePreview || user.avatar)
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-accent">
                    {statsLoading ? "..." : teacherStats.coursesCount}
                  </div>
                  <div className="text-xs text-muted-foreground">Courses</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-accent">
                    {statsLoading ? "..." : teacherStats.studentsCount}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Email cannot be changed as it serves as your unique
                  identifier.
                </p>
              </div>
            </div>

...
from .models import User, Course, Chapter, Quiz, Question, StudentProgress
...
class ChapterTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.chapter_detail_url = reverse('chapters-detail', kwargs={'pk': self.chapter.pk})
...
class QuizTests(APITestCase):
    ...
    def setUp(self):
        self.teacher = User.objects.create_user(email='teacher@example.com', password='password', role='teacher')
        self.student = User.objects.create_user(email='student@example.com', password='password', role='student')
        self.course = Course.objects.create(title='Test Course', teacher=self.teacher, estimated_hours=1)
        self.quiz = Quiz.objects.create(course=self.course, title='Test Quiz', description='A quiz for testing')
        self.question = Question.objects.create(quiz=self.quiz, question='1+1=', correct_answer=1, options=['1', '2'])

    def test_create_quiz_as_teacher(self):
        self.client.force_authenticate(user=self.teacher)
        data = {'course': self.course.pk, 'title': 'New Quiz', 'description': '... A new quiz ...'}
        response = self.client.post(reverse('quizzes-list'), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class StudentProgressTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.student = User.objects.create_user(email='student@example.com', password='password', role='student')
        self.course = Course.objects.create(title='Test Course', teacher=self.teacher, estimated_hours=1)
        self.chapter = Chapter.objects.create(course=self.course, title='Test Chapter', order=1, content='...')
        self.progress = StudentProgress.objects.create(student=self.student, course=self.course)
    ...
    def test_complete_chapter(self):
        self.client.force_authenticate(user=self.student)
        url = reverse('progress-complete-chapter', kwargs={'pk': self.progress.pk})
        response = self.client.post(url, {'chapter_id': self.chapter.pk}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TeacherAPITests(APITestCase):
    ...
    def test_course_analytics_access_teacher(self):
        self.client.force_authenticate(user=self.teacher)
        response = self.client.get(reverse('course_analytics', kwargs={'course_id': self.course.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

...
class StudentProgressViewSet(viewsets.ModelViewSet):
    ...
    def complete_chapter(self, request, pk=None):
        ...
        try:
            chapter = Chapter.objects.get(id=chapter_id, course=progress.course)
...
class TeacherDashboardAPI(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        total_students = StudentProgress.objects.filter(course__teacher=teacher).values('student').distinct().count()
        avg_rating = courses.aggregate(avg_rating=Avg('ratings__rating'))['avg_rating'] or 0
